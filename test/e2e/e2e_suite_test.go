...
var (
	...
	projectImage = "example.com/testplane:v0.0.1"

	// namespace where the project is deployed in
	namespace = "testplane-system"
)
...
func TestE2E(t *testing.T) {
	...
	_, _ = fmt.Fprintf(GinkgoWriter, "Starting testplane integration test suite\n")
...
var _ = BeforeSuite(func() {
	...
	verifyControllerUp := func(g Gomega) {
		...
		g.Expect(output).To(Equal("Running"), "Controller manager pod is not running")
	}
...
var _ = AfterSuite(func() {
	...
	cmd = exec.Command("kubectl", "delete", "clusterrolebinding", "testplane-metrics-binding", "--ignore-not-found")
	_, _ = utils.Run(cmd)
